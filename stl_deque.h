...
struct _Deque_iterator {
  ...
  bool operator<(const _Self& __x) const {
    return (_M_node == __x._M_node) ? (_M_cur < __x._M_cur)
                                    : (_M_node < __x._M_node);
  }
...
void _Deque_base<_Tp, _Alloc>::_M_initialize_map(size_t __num_elements) {
  ...
  __STL_TRY { _M_create_nodes(__nstart, __nfinish); }
  __STL_UNWIND(
...
class deque : protected _Deque_base<_Tp, _Alloc> {
 ...
 public:  // Basic accessors
  ...
  deque_reverse_iterator rbegin() { return deque_reverse_iterator(_M_finish); }
  deque_reverse_iterator rend() { return deque_reverse_iterator(_M_start); }
  ...
  void _M_range_check(size_type __n) const {
    if (__n >= this->size()) __stl_throw_range_error("deque");
  }
 ...
 public:  // Constructor, destructor.
  ...
  void swap(deque& __x) {
    ...
    __STD::swap(this->_M_map, __x._M_map);
 ...
 protected:  // Allocation of _M_map and nodes
  ...
  void _M_reserve_map_at_back(size_type __nodes_to_add = 1) {
    if (__nodes_to_add + 1 > _M_map_size - (_M_finish._M_node - this->_M_map))
      _M_reallocate_map(__nodes_to_add, false);
  }

  void _M_reserve_map_at_front(size_type __nodes_to_add = 1) {
    if (__nodes_to_add > size_type(_M_start._M_node - this->_M_map))
...
template <class _Tp, class _Alloc>
typename deque<_Tp, _Alloc>::iterator deque<_Tp, _Alloc>::erase(
...
void deque<_Tp, _Alloc>::_M_range_initialize(_ForwardIterator __first,
  ...
  __STL_TRY {
    for (__cur_node = _M_start._M_node; __cur_node < _M_finish._M_node;
         ++__cur_node) {
      _ForwardIterator __mid = __first;
  ...
  if (__elems_before < difference_type(__length / 2)) {
    ...
    __STL_TRY {
      if (__elems_before >= difference_type(__n)) {
        ...
        __uninitialized_copy_fill(_M_start, __pos, __new_start, _M_start,
                                  __x_copy);
        _M_start = __new_start;
...
void deque<_Tp, _Alloc>::_M_insert_aux(iterator __pos,
  ...
  if (__elemsbefore < __length / 2) {
    iterator __new_start =
        _M_reserve_elements_at_front(__n);  // may change _M_start and _M_end
    iterator __old_start = _M_start;
    __pos = _M_start + __elemsbefore;
    __STL_TRY {
      if (__elemsbefore >= difference_type(__n)) {
...
void deque<_Tp, _Alloc>::_M_reallocate_map(size_type __nodes_to_add,
  ...
  if (_M_map_size > 2 * __new_num_nodes) {
    __new_nstart = this->_M_map + (_M_map_size - __new_num_nodes) / 2 +
    ...
    __new_nstart = __new_map + (__new_map_size - __new_num_nodes) / 2 +
                   (__add_at_front ? __nodes_to_add : 0);
    copy(_M_start._M_node, _M_finish._M_node + 1, __new_nstart);
    _M_deallocate_map(this->_M_map, _M_map_size);
