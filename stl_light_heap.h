...
#ifndef __SGI_STL_LIGHT_HEAP_H
#define __SGI_STL_LIGHT_HEAP_H
...
struct light_heap {
  ...
  void __push_heap(_RandomAccessIterator __first, _Distance __holeIndex,
                   _Distance __topIndex, const _Tp& __value) {
    //    std::cout << "push_heap2" << std::endl;
  ...
  template <class _RandomAccessIterator, class _Distance, class _Tp,
            class _Compare>
  void __push_heap(_RandomAccessIterator __first, _Distance __holeIndex,
                   _Distance __topIndex, const _Tp& __value, _Compare __comp) {
    _Distance __parent = (__holeIndex - 1) / 2;
    while (__holeIndex > __topIndex && __comp(*(__first + __parent), __value)) {
      //      std::cout << "push_heap2_comp" << std::endl;
      *(__first + __holeIndex) = *(__first + __parent);
      __holeIndex = __parent;
      __parent = (__holeIndex - 1) / 2;
    }
  ...
  void __adjust_heap(_RandomAccessIterator __first, _Distance __holeIndex,
                     _Distance __len, const _Tp& __value) {
    _Distance __topIndex = __holeIndex;
    _Distance __secondChild = 2 * __holeIndex + 2;
    while (__secondChild < __len) {
      if (*(__first + __secondChild) < *(__first + (__secondChild - 1)))
        __secondChild--;
      if (*(__first + __secondChild) < __value) {
        break;
      }
  ...
  inline void __pop_heap(_RandomAccessIterator __first,
                         _RandomAccessIterator __last,
                         _RandomAccessIterator __result, const _Tp& __value,
                         _Distance*) {
    *__result = *__first;
  ...
  template <class _RandomAccessIterator, class _Distance, class _Tp,
            class _Compare>
  void __adjust_heap(_RandomAccessIterator __first, _Distance __holeIndex,
                     _Distance __len, const _Tp& __value, _Compare __comp) {
    _Distance __topIndex = __holeIndex;
    _Distance __secondChild = 2 * __holeIndex + 2;
    while (__secondChild < __len) {
      if (__comp(*(__first + __secondChild), *(__first + (__secondChild - 1))))
        __secondChild--;
      if (__comp(*(__first + __secondChild), __value)) {
        *(__first + __holeIndex) = __value;
        return;
      }
      *(__first + __holeIndex) = *(__first + __secondChild);
  ...
  inline void make_heap(_RandomAccessIterator __first,
    ...
    __STL_REQUIRES(typename iterator_traits<_RandomAccessIterator>::value_type,
                   _LessThanComparable);
    //    std::cout << "hello make heap1" << std::endl;
  ...
  inline void make_heap(_RandomAccessIterator __first,
                        _RandomAccessIterator __last, _Compare __comp) {
    //    std::cout << "hello make heap2" << std::endl;
