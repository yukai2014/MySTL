...
class vector : protected _Vector_base<_Tp, _Alloc> {
 ...
 public:
  ...
  vector_reverse_iterator rbegin() { return vector_reverse_iterator(end()); }
  const_reverse_iterator rbegin() const {
    return const_reverse_iterator(end());
  }
  vector_reverse_iterator rend() { return vector_reverse_iterator(begin()); }
  ...
  size_type capacity() const { return size_type(-begin()); }
  ...
  void _M_range_check(size_type __n) const {
    if (__n >= this->size()) __stl_throw_range_error("vector");
  }
  ...
  void reserve(size_type __n) {
    if (capacity() < __n) {
      ...
      _M_deallocate(this->_M_start, -this->_M_start);
      this->_M_start = __tmp;
  ...
  iterator erase(iterator __first, iterator __last) {
    iterator __i = copy(__last, this->_M_finish, __first);
    destroy(__i, this->_M_finish);
...
void vector<_Tp, _Alloc>::_M_assign_aux(_ForwardIter __first,
  ...
  if (__len > capacity()) {
    ...
    _M_deallocate(_M_start, this->_M_end_of_storage - _M_start);
    _M_start = __tmp;
    this->_M_end_of_storage = _M_finish = _M_start + __len;
  } else if (size() >= __len) {
...
void vector<_Tp, _Alloc>::_M_insert_aux(iterator __position, const _Tp& __x) {
  if (this->_M_finish != this->_M_end_of_storage) {
    construct(this->_M_finish, *(this->_M_finish - 1));
    ++this->_M_finish;
    _Tp __x_copy = __x;
  ...
  if (__n != 0) {
    if (size_type(this->_M_end_of_storage - this->_M_finish) >= __n) {
      ...
      if (__elems_after > __n) {
        uninitialized_copy(this->_M_finish - __n, this->_M_finish,
                           this->_M_finish);
        this->_M_finish += __n;
...
void vector<_Tp, _Alloc>::_M_range_insert(iterator __position,
  ...
  if (__first != __last) {
    size_type __n = 0;
    distance(__first, __last, __n);
    if (size_type(this->_M_end_of_storage - _M_finish) >= __n) {
  ...
  if (__first != __last) {
    ...
    if (size_type(this->_M_end_of_storage - this->_M_finish) >= __n) {
      ...
      if (__elems_after > __n) {
        ...
        uninitialized_copy(__first + __elems_after, __last, this->_M_finish);
        this->_M_finish += __n - __elems_after;
        uninitialized_copy(__position, __old_finish, this->_M_finish);
        this->_M_finish += __elems_after;
